...
from probabilistic_switch import ProbabilisticSoftSwitch
...
class CompleteIntegratedTradingSystem:
    ...
    def __init__(self, initial_balance=100000, config=None):
        ...
        self.prob_switch = ProbabilisticSoftSwitch()
    ...
    def initialize_system(self, data):
        ...
        # 3. 概率软切换初始化
        print("3. 初始化概率切换...")
        self.prob_switch.initialize_strategies(list(self.strategies.keys()))
        self.prob_switch.classifier.fit(X, self.model_optimizer.prepare_labels(y))
    ...
    def process_market_data(self, data):
        ...
        # 2. 市场类型预测（概率）
        market_probs = self.prob_switch.classifier.predict_proba(X.tail(1))
        
        # 3. 概率软切换 - 更新策略权重
        strategy_mapping = {
            'grid_trading': ['range_bound', 'volatile'],
            'trend_following': ['trending_up', 'trending_down'],
            'mean_reversion': ['range_bound'],
            'volatility_arb': ['volatile']
        }
        
        self.strategy_weights = self.prob_switch.update_weights(
            market_probs, strategy_mapping
        )
        ...
        # 6. 风控检查 - ATR止损+风险止损
        atr = self.atr_stop.atr_manager.atr_calc.calculate_atr(df).iloc[-1]
        self.atr_stop.enter_position(current_price, 'long', df)
        stop_result = self.atr_stop.update(current_price, df)
    ...
    def _execute_decision(self, decision, current_price, atr):
        ...
        if action == 'buy' and self.position == 0:
            ...
            # 交易成本优化
            if self.config['use_cost_optimization']:
                chunks, _ = self.cost_optimizer.optimize_execution(
                    position_size, current_price, 100000
                )
            else:
                chunks = [position_size]
            
            total_cost = 0
            for chunk in chunks:
                cost = self.cost_optimizer.calculate_total_cost(
                    chunk, current_price, is_sell=False
                )
                total_cost += cost
            ...
            self.trade_history.append({
                'date': df.index[-1] if 'df' in locals() else pd.Timestamp.now(),
                'type': 'sell',
                'price': current_price,
