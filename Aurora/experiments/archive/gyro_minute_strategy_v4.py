...
class GyroEvolve:
    ...
    def get_reward(self, metrics: StrategyMetrics, position_change):
        ...
        if metrics.sortino_ratio >= 1.2:
            reward += metrics.sortino_ratio * 12
        else:
            reward -= (1.2 - metrics.sortino_ratio) * 8
...
class GyroMinuteStrategyV4:
    ...
    def should_trade(self, adjust, price_window, min_holding_minutes=25):
        if abs(adjust.main_pos) < 0.0006:
            return False
    ...
    def run_backtest(self, data, initial_capital=100000):
        ...
        for i in range(MINUTE_WINDOW_LONG, len(prices)):
            ...
            if self.should_trade(adjust, short_window, min_holding_minutes=25):
...
def main():
    print("="*90)
    print("陀螺策略V4 - 温和优化版本")
    print("改进: 提高Sortino激励、降低信号阈值增加交易频率、小幅优化其他参数")
    print("="*90)
