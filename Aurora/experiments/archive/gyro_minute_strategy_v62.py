...
class GyroEvolve:
    ...
    def get_reward(self, metrics: StrategyMetrics, position_change):
        ...
        if metrics.sortino_ratio >= 3.0:
            ...
            reward += metrics.sortino_ratio * 4
        else:
            reward -= (1.5 - metrics.sortino_ratio) * 10
...
class GyroMinuteStrategyV62:
    ...
    def should_trade_multicycle(self, adjust, price_window, min_holding_minutes=30):
        ...
        mom_60 = self.dynamics.indicator.calc_momentum_minute(price_window, 60)
        signals = [np.sign(mom_15), np.sign(mom_60)]
        agreement = sum(1 for s in signals if s == np.sign(momentum))
        if agreement < 1:  # 至少1个周期确认
            return False
    ...
    def run_backtest(self, data, initial_capital=100000):
        ...
        for i in range(MINUTE_WINDOW_LONG, len(prices)):
            ...
            # 参数矩阵收敛更新 - V6.2新增
            current_score = 7.5 + min(2.0, reward / 15)
            self.convergence.update(new_omega, current_score)
            ...
            if self.should_trade_multicycle(adjust, short_window, min_holding_minutes=30):
...
if __name__ == "__main__":
    print("="*90)
    print("陀螺恒稳进动矩阵·V6.2增强版（最小化改进）")
    print("改进: 增强Sortino激励 + 参数矩阵收敛 + 多周期确认")
    print("="*90)
    ...
    strategy = GyroMinuteStrategyV62()
    result = strategy.run_backtest(test_data, 100000)
    
    print("\n" + "="*90)
    print("[回测结果] V6.2增强版回测结果")
    print("="*90)
    ...
    print(f"\n[参数收敛] 状态: {'已收敛' if converged else '未收敛'}")
    print(f"平均收敛分数: {avg_score:.2f}")
    if strategy.convergence.best_params is not None:
        print(f"最优参数组合: {strategy.convergence.best_params}")
