...
MAX_LEVERAGE = 4.0  # 提升杠杆上限
SINGLE_ASSET_MAX = 0.4
DT = 1 / (252 * 24 * 60)

MINUTE_WINDOW_SHORT = 30     # 缩短窗口提高响应
...
class GyroEvolveV6:
    def __init__(self, initial_omega=None):
        if initial_omega is None:
            self.omega = np.array([0.45, 0.35, 0.20])  # 优化初始值
        else:
    ...
    def sample_experience(self, batch_size=BATCH_SIZE):
    ...
    def get_reward(self, metrics, position_change):
        ...
        if metrics.sortino_ratio >= 3.0:
            reward += metrics.sortino_ratio * 20
        elif metrics.sortino_ratio >= 2.0:
            reward += metrics.sortino_ratio * 15
        elif metrics.sortino_ratio >= 1.5:
            reward += metrics.sortino_ratio * 10
        else:
            reward -= (1.5 - metrics.sortino_ratio) * 12
        ...
        if TARGET_SHARPE_MIN <= metrics.sharpe_ratio <= TARGET_SHARPE_MAX:
            ...
            reward -= abs(metrics.sharpe_ratio - TARGET_SHARPE_MIN) * 6
        
        # 3. K比率
        if metrics.k_ratio > TARGET_KRATIO:
            reward += metrics.k_ratio * 10
        
        # 4. 最大回撤惩罚
        reward -= metrics.max_dd * 40
        
        # 5. 李雅普诺夫惩罚
        reward -= abs(metrics.lyapunov) * 35
        
        # 6. Omega比率
        if metrics.omega_ratio >= 1.5:
            reward += metrics.omega_ratio * 6
        
        # 7. 尾部比率
        if metrics.tail_ratio >= 1.5:
            reward += metrics.tail_ratio * 5
    ...
    def evolve_omega(self, state, reward):
        ...
        if self.target_update_counter >= TARGET_UPDATE_FREQ:
            self.target_omega = self.omega.copy() * 0.9 + new_omega * 0.1
            self.target_update_counter = 0
...
class AdaptiveRiskManager:
    def __init__(self):
        self.base_stop_loss = 0.015
        self.base_take_profit = 0.03
    ...
    def calculate_risk_params(self, atr, volatility_level, trend_strength):
        ...
        take_profit = self.base_take_profit + atr * self.atr_multiplier * 1.8
        ...
        stop_loss = np.clip(stop_loss, 0.008, 0.035)
        take_profit = np.clip(take_profit, 0.015, 0.07)
        
        return stop_loss, take_profit

    def should_stop(self, current_price, entry_price, position_sign, stop_loss, take_profit):
        if position_sign == 0:
            return False, None
        
        ret = (current_price - entry_price) / entry_price * position_sign
        if ret <= -stop_loss:
...
# V6陀螺动力学（保留原核心）
# ==============================
class GyroDynamicsV6:
    def __init__(self):
        self.dt = DT
        self.indicator = GyroIndicatorV6()
...
# V6完整策略主类
# ==============================
class GyroMinuteStrategyV6:
    def __init__(self):
        self.dynamics = GyroDynamicsV6()
        self.evolver = GyroEvolveV6()
        self.opt = GyroPortfolioOptV6()
    ...
    def run_backtest(self, data, initial_capital=100000):
        ...
        for i in range(MINUTE_WINDOW_LONG, len(prices)):
            ...
            # 自适应信号阈值
            signal_threshold = max(0.0002, min(0.0008, 0.0004 + vol * 0.02))
            ...
            # 参数矩阵收敛更新
            self.convergence.update(new_omega, 8.0 + min(2.0, reward / 20))
            results['convergence_history'].append({
                'gen': i,
                'score': 8.0 + min(2.0, reward / 20),
            ...
            if should_open:
                ...
                position_size = np.sign(adjust.main_pos) * min(1.5, signal_strength * 18) * (equity / prices[i])
        ...
        if isinstance(eval_result, tuple) and len(eval_result) >= 2:
            score = eval_result[0]
            metric_scores = eval_result[1] if len(eval_result) > 1 else {}
        elif isinstance(eval_result, dict):
            score = eval_result.get('score', 0.0)
            metric_scores = eval_result
        else:
        ...
        results.update({
            ...
            'max_drawdown_pct': max_dd,
            'total_trades': len(self.trade_history),
            'metric_scores': metric_scores if isinstance(metric_scores, dict) else {}
        })
...
def main():
    print("="*90)
    print("陀螺恒稳进动矩阵策略 - V6增强版")
    print("改进：参数矩阵收敛机制 + 强化学习经验回放 + 自适应风控 + 多周期确认")
    print("="*90)
    ...
    print("\n指标得分：")
    mscores = result.get('metric_scores', {})
    if mscores:
        for k, v in sorted(mscores.items(), key=lambda x: x[1], reverse=True):
            try:
                vv = float(v)
                status = "[OK]" if vv >= 8 else "[WARN]" if vv >= 6 else "[FAIL]"
                print(f"{status} {k:<25}: {vv:.1f}")
            except Exception:
                print(f"[INFO] {k:<25}: {v}")
    else:
        print("  (本地评估器未提供分项得分)")
