...
def generate_large_market_data(days=100, time_frame=5):
    ...
    # 市场阶段划分
    phases = [
        {"type": "sideway", "days": 25, "volatility": 0.2},
        {"type": "up", "days": 25, "volatility": 0.15, "trend": 0.3},
        {"type": "down", "days": 25, "volatility": 0.15, "trend": -0.25},
        {"type": "volatility", "days": 25, "volatility": 0.4}
    ]
    ...
    for phase in phases:
        ...
        for i in range(phase_minutes):
            if phase["type"] == "sideway":
                ...
                current_price += noise
            elif phase["type"] == "down":
                # 下跌市场
                trend = phase["trend"] / phase_minutes
                noise = np.random.normal(0, phase["volatility"])
                current_price *= (1 + trend)
                current_price += noise
            elif phase["type"] == "volatility":
                # 高波动市场
                noise = np.random.normal(0, phase["volatility"])
                current_price += noise
