...
try:
    ...
    from strategies.downward_grid import DownwardGridTrading
    from strategies.reversal_grid import ReversalGridTrading
    from strategies.ml_reversal_grid import MLReversalGridTrading
    from strategies.optimized_downward_reversal import OptimizedDownwardReversalGrid
    from strategies.final_adaptive_grid import FinalAdaptiveGridTrading
...
def test_strategy(strategy_name: str, strategy_class, market_type: str, initial_balance: float = 100000):
    ...
    # 创建策略实例
    if strategy_name in ['ml_grid_trading', 'downward_grid', 'reversal_grid', 'ml_reversal_grid', 'optimized_downward_reversal', 'ml_range_grid', 'adaptive_range_grid']:
        strategy = strategy_class(base_price=data.iloc[0], initial_balance=initial_balance)
    elif strategy_name in ['final_adaptive_grid', 'final_market_adaptive']:
        strategy = strategy_class(base_price=data.iloc[0], initial_balance=initial_balance)
    else:
    ...
    for i, price in enumerate(data):
        ...
        if i >= 20:
            ...
            if strategy_name in ['ml_grid_trading', 'enhanced_grid', 'adaptive_grid', 'reversal_grid', 'ml_reversal_grid', 'final_adaptive_grid', 'final_market_adaptive', 'optimized_downward_reversal']:
...
def main():
    ...
    strategies = {
        ...
        'downward_grid': DownwardGridTrading,
        'reversal_grid': ReversalGridTrading,
        'ml_reversal_grid': MLReversalGridTrading,
        'optimized_downward_reversal': OptimizedDownwardReversalGrid,
        'final_adaptive_grid': FinalAdaptiveGridTrading,
    ...
    for market_type in market_types:
        ...
        for result in market_results:
            print(f"  {result['strategy']:25s} 收益率: {result['return_rate']:8.2f}% 交易次数: {result['total_trades']:5d} 胜率: {result['win_rate']:5.2%}")
    ...
    # 上涨市场最佳策略
    up_market_results = [r for r in all_results if r['market_type'] == 'trending_up']
    if up_market_results:
        best_up = max(up_market_results, key=lambda x: x['return_rate'])
        print(f"\n" + "="*100)
        print("上涨市场最佳策略")
        print("="*100)
        print(f"  策略: {best_up['strategy']}")
        print(f"  收益率: {best_up['return_rate']:.2f}%")
        print(f"  交易次数: {best_up['total_trades']}")
        print(f"  胜率: {best_up['win_rate']:.2%}")
    
    # 横盘市场最佳策略
    range_market_results = [r for r in all_results if r['market_type'] == 'range_bound']
    if range_market_results:
        best_range = max(range_market_results, key=lambda x: x['return_rate'])
        print(f"\n" + "="*100)
        print("横盘市场最佳策略")
        print("="*100)
        print(f"  策略: {best_range['strategy']}")
        print(f"  收益率: {best_range['return_rate']:.2f}%")
        print(f"  交易次数: {best_range['total_trades']}")
        print(f"  胜率: {best_range['win_rate']:.2%}")
    
    # 波动市场最佳策略
    volatile_market_results = [r for r in all_results if r['market_type'] == 'volatile']
    if volatile_market_results:
        best_volatile = max(volatile_market_results, key=lambda x: x['return_rate'])
        print(f"\n" + "="*100)
        print("波动市场最佳策略")
        print("="*100)
        print(f"  策略: {best_volatile['strategy']}")
        print(f"  收益率: {best_volatile['return_rate']:.2f}%")
        print(f"  交易次数: {best_volatile['total_trades']}")
        print(f"  胜率: {best_volatile['win_rate']:.2%}")
