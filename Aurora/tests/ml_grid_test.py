...
def test_ml_grid_trading(market_type: str, initial_balance: float = 100000):
    ...
    """
    print(f"\n" + "="*60)
    print(f"测试: {market_type} 市场 (机器学习优化)")
    print("="*60)
    ...
    # 创建网格步长优化器
    optimizer = GridStepOptimizer()
    
    # 创建网格交易策略
    grid = GridTrading(
        base_price=data.iloc[0], 
        grid_spacing=0.003,  # 更小的初始网格间距
    ...
    for i, price in enumerate(data):
        ...
        if i >= 20 and i % 10 == 0:
            ...
            # 使用机器学习预测最优网格步长
            optimal_step = optimizer.calculate_optimal_grid_step(current_data)
            grid.grid_spacing = optimal_step
            grid.grids = grid._create_grids()
            step_history.append(optimal_step)
    ...
    if step_history:
        print(f"  平均网格步长: {np.mean(step_history):.4f}")
        print(f"  最小网格步长: {min(step_history):.4f}")
        print(f"  最大网格步长: {max(step_history):.4f}")
