...
def generate_market_data(length: int, market_type: str = 'range_bound', start_price: float = 100):
    ...
    if market_type == 'range_bound':
        ...
        random_noise = np.random.normal(0, 0.009, length)
        returns = trend + cycle + random_noise
        prices = start_price * (1 + returns).cumprod()
        ...
        random_noise = np.random.normal(0, 0.012, length)
        returns = trend + cycle + random_noise
        prices = start_price * (1 + returns).cumprod()
...
def test_enhanced_grid_trading(market_type: str, initial_balance: float = 100000):
    """
    测试增强版网格交易策略
    """
    print(f"\n" + "="*60)
    print(f"测试: {market_type} 市场 (增强版网格交易)")
    print("="*60)
    ...
    print(f"  网格调整次数: {perf['grid_adjustments']}")
...
def main():
    ...
    for market_type in market_types:
        result = test_enhanced_grid_trading(market_type)
    ...
    for result in results:
        print(f"{result['market_type']:15s} 收益率: {result['return_rate']:8.2f}% 交易次数: {result['total_trades']:5d} 胜率: {result['win_rate']:5.2%} 调整次数: {result['grid_adjustments']:3d}")
