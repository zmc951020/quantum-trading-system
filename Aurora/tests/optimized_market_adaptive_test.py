...
from strategies.optimized_market_adaptive import OptimizedMarketAdaptiveStrategy
...
def test_strategy(strategy, data, market_name):
    ...
    for i, (index, row) in enumerate(data.iterrows()):
        ...
        if result['action'] == 'buy':
            buy_count += 1
            print(f"  买入: {result['quantity']:.2f} @ {result['price']:.2f} 余额: {result['balance']:.2f}")
    ...
    print(f"  策略切换次数: {performance['strategy_switch_count']}")
...
def main():
    ...
    for market_type in market_types:
        ...
        # 测试优化的市场自适应策略
        strategy = OptimizedMarketAdaptiveStrategy(base_price=data['close'].iloc[0])
        results[market_type] = test_strategy(strategy, data, f"optimized_market_adaptive")
    ...
    for market_type in market_types:
        ...
        print(f"  策略切换次数: {perf['strategy_switch_count']:8}")
