...
SLIPPAGE = 0.0001           # 滑点
MAX_DAILY_TRADES = 50       # 降低交易频率
MAX_SINGLE_LOSS = 0.01      # 增加单笔最大亏损
MAX_DRAW_DOWN_LIMIT = 0.05  # 增加最大回撤限制
MODEL_PATH = "positive_profitable_strategy_ml_memory.json"

# 最小交易盈利阈值，确保收益覆盖手续费
MIN_PROFIT_THRESHOLD = FEE_RATE * 2  # 至少覆盖2倍手续费
...
if __name__ == "__main__":
    ...
    for i in range(28800):
        # 生成价格，包含上涨趋势和小幅波动
        trend = 0.0001  # 每日上涨约1.4%
        volatility = 0.0002  # 小幅波动
    ...
    high_price = price * (1 + np.random.uniform(0, 0.0002, 28800))
    low_price = price * (1 - np.random.uniform(0, 0.0002, 28800))
    close_price = price
