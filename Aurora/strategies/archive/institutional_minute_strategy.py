...
ML_FILE = "ml_institutional_memory.json"
...
class MLPersistentLearner:
    ...
    def load(self):
        ...
        return {
            ...
            "best_position": {"RANGE":0.9, "STRONG_UP":0.8, "STRONG_DOWN":0.3},
            "best_params":   {"RANGE": {"atr_multiplier": 0.1, "rsi_buy": 45, "rsi_sell": 55},
                              "STRONG_UP": {"rsi_max": 80},
                              "STRONG_DOWN": {"rsi_buy": 30, "rsi_sell": 70}},
            "market_return": {},
    ...
    def get_optimal_position(self, market_type):
        return self.data["best_position"].get(market_type, 0.4)

    def get_optimal_params(self, market_type):
        return self.data["best_params"].get(market_type, {})
...
def auto_classify_market(df):
    ...
    if adx_last > 20 and trend > 0.005:
        return "STRONG_UP"
    if adx_last > 20 and trend < -0.005:
        return "STRONG_DOWN"
    if vol_last > 0.15:
...
# ===================== 【核心3】策略自动切换 =====================
def auto_strategy_signal(df, market, ml):
    ...
    params = ml.get_optimal_params(market)

    # 横盘：ATR动态网格（最赚钱）
    if market == "RANGE":
        atr_multiplier = params.get("atr_multiplier", 0.1)
        rsi_buy = params.get("rsi_buy", 45)
        rsi_sell = params.get("rsi_sell", 55)
        ma20 = np.mean(close[-20:])
        if last < ma20 - atr_multiplier*atr and rsi < rsi_buy:
            return 1
        if last > ma20 + atr_multiplier*atr and rsi > rsi_sell:
            return -1

    # 上涨：趋势跟随
    if market == "STRONG_UP":
        rsi_max = params.get("rsi_max", 80)
        if last > close[-2] and rsi < rsi_max:
            return 1
    ...
    if market == "STRONG_DOWN":
        ...
        if last < np.min(close[-10:]) and rsi < rsi_buy:
            return 1
        if last > np.max(close[-10:]) and rsi > rsi_sell:
            return -1
    return 0

# ===================== 【核心4】ML优化仓位&步长 =====================
def ml_optimized_position(market, capital, price, atr, ml):
    mul = ml.get_optimal_position(market)
...
# ===================== 【核心5】交易执行引擎 =====================
class InstitutionalMinuteStrategy:
    ...
    def close_all(self, price, market_type):
        if self.pos == 0:
            return
        initial_balance = self.capital + self.pos * self.cost_price
        if self.pos > 0:
        ...
        if pnl > 0:
            self.win_count += 1
            if market_type in self.market_type_analysis:
                self.market_type_analysis[market_type]["wins"] += 1
        if market_type in self.market_type_analysis:
            self.market_type_analysis[market_type]["trades"] += 1
            self.market_type_analysis[market_type]["return"] += (self.capital - initial_balance)
        self.pos = 0
        self.cost_price = 0
        self.trade_count += 1

    def open_long(self, price, qty, market_type):
    ...
    def update_price(self, current_price, data):
        ...
        signal = auto_strategy_signal(data, market, self.ml)
        atr = ATR(data.high, data.low, data.close).iloc[-1]
        pos_size = ml_optimized_position(market, self.capital, current_price, atr, self.ml)

        # 执行交易
        if signal == 1 and self.pos <= 0:
            self.close_all(current_price, market)
            self.open_long(current_price, pos_size, market)
            return {
                "action": "buy",
                "quantity": pos_size,
                "price": current_price,
...
if __name__ == "__main__":
    ...
    # 初始化策略
    strategy = InstitutionalMinuteStrategy()
