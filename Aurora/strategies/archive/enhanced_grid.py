...
class EnhancedGridTrading:
    ...
    def __init__(self, base_price: float, grid_spacing: float = 0.002, initial_balance: float = 100000):
        ...
        self.max_position_percentage = 0.75  # 最大持仓比例
        self.reserve_balance_percentage = 0.25  # 保留资金比例
    ...
    def update_price(self, current_price: float, data: pd.Series = None) -> Dict[str, any]:
        ...
        if len(self.price_history) > 30:
            # 短期波动率（15分钟）
            short_term_prices = pd.Series(self.price_history[-15:])
            short_term_volatility = short_term_prices.pct_change().std()
            
            # 中期波动率（30分钟）
            medium_term_prices = pd.Series(self.price_history[-30:])
            medium_term_volatility = medium_term_prices.pct_change().std()
            
            # 综合考虑短期和中期波动率
            volatility = (short_term_volatility + medium_term_volatility) / 2
            
            if volatility > 0:
                # 波动市场中，网格间距与波动率成正比
                optimal_spacing = max(self.min_grid_spacing, min(self.max_grid_spacing, volatility * 2.5))
        ...
        if grid_change < 0 and self.position < max_position:
            # 价格下跌到更低网格 -> 买入（低买）
            # 计算买入金额：基于网格变化、可用资金和波动率
            base_buy_amount = min(abs(grid_change) * 800, available_balance * 0.15)
            # 波动率越高，买入金额越小，避免过度交易
            buy_amount = base_buy_amount * (1 - min(volatility * 10, 0.5))
            ...
            # 价格上涨到更高网格 -> 卖出（高卖）
            # 计算卖出数量：基于网格变化、当前持仓和波动率
            base_sell_quantity = min(abs(grid_change) * 800 / current_price, self.position * 0.7)
            # 波动率越高，卖出比例越大，锁定利润
            sell_quantity = base_sell_quantity * (1 + min(volatility * 10, 0.3))
