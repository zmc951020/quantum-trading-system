...
SINGLE_RISK = 0.005
MAX_DD = 0.08
ML_FILE = "ml_covariance_opt.json"
...
def get_market(df):
    ...
    if adx.iloc[-1]>24 and trend>0.01:
        return "STRONG_UP"
    if adx.iloc[-1]>24 and trend<-0.01:
        return "STRONG_DOWN"
    if vol.iloc[-1]>0.22:
...
def signal(df, market, ml):
    ...
    if market == "RANGE":
        ...
        if last < ma20 - opt["step"]*atr and rsi<45:
            return 1
        if last > ma20 + opt["step"]*atr and rsi>55:
            return -1
    
    if market == "STRONG_UP":
        if last > close[-2] and rsi<75:
            return 1
...
# ===================== 交易引擎 =====================
class CovarianceInstitutionalStrategy:
...
if __name__ == "__main__":
    ...
    # 初始化策略
    strategy = CovarianceInstitutionalStrategy()
