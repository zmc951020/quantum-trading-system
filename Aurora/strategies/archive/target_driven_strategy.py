...
def auto_strategy_signal(df, market):
    ...
    if market == "RANGE":
        ...
        if last < ma20 - 0.05*atr and rsi < 45:
            return 1
        if last > ma20 + 0.05*atr and rsi > 55:
            return -1

    # 上涨：趋势跟随
    if market == "STRONG_UP":
        if last > close[-2] and rsi < 85:
            return 1

    # 下跌：支撑压力 + 黑洞反转 + 防御
    if market == "STRONG_DOWN":
        if last < np.min(close[-5:]) and rsi < 30:
            return 1
        if last > np.max(close[-5:]) and rsi > 70:
            return -1
    return 0
...
def ml_optimized_position(market, capital, price, atr):
    mul = {
        "RANGE": 0.8, "STRONG_UP": 0.7,
        "STRONG_DOWN": 0.2, "PANIC": 0.0
    }.get(market, 0.3)
