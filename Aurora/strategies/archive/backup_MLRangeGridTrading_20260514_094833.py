...
class MLRangeGridTrading:
    ...
    def update_price(self, current_price: float, data: pd.Series = None) -> Dict[str, any]:
        ...
        # 更新最高余额
        if total_value > self.highest_balance:
            self.highest_balance = total_value
        
        # 计算当前回撤
        drawdown = (self.highest_balance - total_value) / self.highest_balance if self.highest_balance > 0 else 0
        ...
        if len(self.profit_history) > 1:
            ...
            std_downside = np.std(downside_returns) if len(downside_returns) > 0 else 1
            
            # 假设无风险利率为0
            self.sharpe_ratio = avg_return / std_return if std_return > 0 else 0
            self.sortino_ratio = avg_return / std_downside if std_downside > 0 else 0
