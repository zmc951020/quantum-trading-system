...
def test_strategy(strategy, data):
    ...
    # 记录各市场类型的交易情况
    market_type_stats = {
        'range_bound': {'trades': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'balance': 100000},
        'trending_up': {'trades': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'balance': 100000},
        'trending_down': {'trades': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'balance': 100000},
        'volatile': {'trades': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'balance': 100000}
    }
    ...
    for i in range(len(data)):
        ...
        prev_balance = strategy.current_balance
            ...
            if market_type in market_type_stats:
                ...
                if result['action'] == 'sell' and prev_position > 0:
                    ...
                    if profit > 0:
                        ...
                        market_type_stats[market_type]['losses'] += 1
    
    # 平仓所有持仓
    if strategy.position > 0:
        current_price = data['close'].iloc[-1]
        revenue = strategy.position * current_price
        strategy.current_balance += revenue
        quantity = strategy.position
        strategy.position = 0
        strategy.entry_price = 0
        strategy.total_trades += 1
        if current_price > strategy.entry_price:
        ...
        if market_type in market_type_stats:
            market_type_stats[market_type]['trades'] += 1
            profit = (current_price - strategy.entry_price) * quantity
            market_type_stats[market_type]['profit'] += profit
            if profit > 0:
                market_type_stats[market_type]['wins'] += 1
            else:
    ...
    for market_type, stats in market_type_stats.items():
        if stats['trades'] > 0:
            ...
            # 计算收益率
            stats['return'] = stats['profit'] / initial_balance
        else:
            stats['win_rate'] = 0
            stats['return'] = 0
...
if __name__ == "__main__":
    ...
    # 输出各市场类型的表现
    print("各市场类型表现分析:")
    print(f"{'市场类型':<15} {'交易次数':>10} {'胜率':>8} {'收益率':>12}")
    print("-"*60)
    
    for market_type, stats in result['market_type_stats'].items():
        print(f"{market_type:<15} {stats['trades']:>10} {stats['win_rate']*100:>8.2f}% {stats['return']*100:>12.2f}%")
