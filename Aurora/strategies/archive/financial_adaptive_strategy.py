...
SLIPPAGE = 0.0001           # 滑点
MAX_DAILY_TRADES = 60       # 高频上限，提高交易频率
MAX_SINGLE_LOSS = 0.015     # 单笔最大亏损1.5%
MAX_DRAW_DOWN_LIMIT = 0.12  # 最大回撤12%
MODEL_PATH = "strategy_ml_memory.json"
    ...
    def get(df, market):
        ...
        if market == "STRONG_UP":
            ...
            if close[-1] > ma10 and ma10 > ma20:
                # 价格回调到MA10附近
                if abs(close[-1] - ma10) / ma10 < 0.01 or close[-1] > close[-2]:
                    return 1
        elif market == "WEAK_UP":
            # 弱势上涨市场：轻仓趋势跟踪
            if close[-1] > ma20 or close[-1] > close[-2]:
                return 1
            ...
            # 窄幅震荡市场：高抛低吸
            if close[-1] < ma20 - std20 * 0.2 or close[-1] > close[-2]:
                return 1
            elif close[-1] > ma20 + std20 * 0.2 or close[-1] < close[-2]:
                return -1
        elif market == "WIDE_RANGE":
            # 宽幅震荡市场：区间突破
            if close[-1] > high[-10:].max() or vol[-1] > np.mean(vol[-5:]):
                return 1
            elif close[-1] < low[-10:].min() or vol[-1] > np.mean(vol[-5:]):
                return -1
        elif market == "LOW_VOL":
            # 低波动市场：突破策略
            if close[-1] > ma10 * 1.001 or vol[-1] > np.mean(vol[-5:]) * 1.05:
                return 1
            elif close[-1] < ma10 * 0.999 or vol[-1] > np.mean(vol[-5:]) * 1.05:
                return -1
...
class FinancialAdaptiveStrategy:
    ...
    def run(self, df):
        ...
        if signal != 0:
            ...
            if trade_cap + fee <= self.capital:
                # 调整仓位大小，确保资金安全
                adjusted_position = min(position, 0.2)  # 单次最大仓位20%
                if signal == 1:
        ...
        if self.pos == 0 and self.capital > INIT_CAPITAL * 0.5:
            # 基于市场类型的默认交易策略
            adjusted_position = min(position, 0.15)  # 单次最大仓位15%
            if market == "STRONG_UP":
                # 强势上涨市场，轻仓买入
                self.buy(price, adjusted_position)
                self.daily_trade_count += 1
