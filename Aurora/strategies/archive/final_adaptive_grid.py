...
class FinalAdaptiveGridTrading:
    """
    最终自适应网格交易策略
    """
    ...
    def __init__(self, base_price: float, initial_balance: float = 100000):
        ...
        self.grid_spacing = 0.002  # 初始网格间距
        ...
        self.max_position_percentage = 0.4  # 最大持仓比例
        self.reserve_balance_percentage = 0.5  # 保留资金比例
    ...
    def _train_models(self):
        ...
        if len(self.model_data) >= 100 or len(self.reversal_data) >= 100:
        ...
        if data is not None:
            ...
            if self.last_market_type != self.market_type:
                if self.market_type == 'range_bound':
                    ...
                    self.max_position_percentage = 0.4
                    self.reserve_balance_percentage = 0.5
                    self.stop_loss_threshold = 0.015
                    self.take_profit_threshold = 0.025
                self.grids = self._create_grids()
