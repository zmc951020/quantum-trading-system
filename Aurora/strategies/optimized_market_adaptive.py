...
class OptimizedMarketAdaptiveStrategy:
    """
    优化的市场自适应策略
    解决策略切换频繁、资金使用率不高的问题
    """
    ...
    def __init__(self, base_price: float, initial_balance: float = 100000):
        ...
        self.market_type_stability = 0  # 市场类型稳定性计数器
        self.min_stability_threshold = 3  # 最小稳定性阈值，低于此值不切换策略
        ...
        self.max_trade_amount = 5000  # 最大交易金额
    ...
    def _select_optimal_strategy(self, market_type: str) -> str:
        """
        选择最优策略
    ...
    def update_price(self, current_price: float, data: pd.Series = None) -> Dict[str, any]:
        ...
        # 检测市场类型
        if data is not None:
            new_market_type = self.detect_market_type(data)
            
            # 更新市场类型稳定性
            if new_market_type == self.market_type:
                self.market_type_stability += 1
            else:
                self.market_type_stability = 1
            
            # 只有当市场类型稳定时才切换策略
            if self.market_type_stability >= self.min_stability_threshold:
                self.last_market_type = self.market_type
                self.market_type = new_market_type
                
                # 选择最优策略
                selected_strategy = self._select_optimal_strategy(self.market_type)
                ...
                if self.current_strategy != new_strategy:
                    # 确保新策略的资金与当前一致
                    new_strategy.current_balance = self.current_balance
