<#
.SYNOPSIS
Ollama MemX 一键自动化部署脚本（工业级规范版）
.DESCRIPTION
自动解决所有环境问题，100%兼容Windows，符合PowerShell规范
#>
...
# ====================== 0. 基础检查 ======================
Write-Host "`n============================================================="
Write-Host "🚀 Ollama MemX 一键自动化部署（规范修复版）"
...
if (-not (Test-Path "requirements.txt") -or -not (Test-Path "docker-compose.yml")) {
    Write-Error "请在 MemX-Ollama 目录下运行此脚本！"
...
$services = @(
    ...
    @{name="Neo4j"; url="http://localhost:7474/health"; timeout=120},  # 修复路径
